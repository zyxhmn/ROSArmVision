...
class PandaGraspNode:
    def __init__(self):
        ...
        # 预定义测试抓取点
        self.test_position = PointStamped()
        self.test_position.header.frame_id = "world"
        self.test_position.point.x = 0.209639
        self.test_position.point.y = 0.258602
        self.test_position.point.z = 0.398231

        # 预定义放置点
        self.target_pose = geometry_msgs.msg.Pose()
        self.target_pose.position.x = 0.22
        self.target_pose.position.y = 0.52
        ...
        rospy.loginfo("Panda抓取节点初始化完成...")

    def start(self):
        # 询问是否为测试模式
        response = input("是否使用测试模式进行抓取? (y/n): ").strip().lower()
        if response == 'y':
            self.test_mode = True
            self.target_position = self.test_position.point
            rospy.loginfo("使用测试模式的固定抓取点")
            self.execute_grasp()
        else:
            self.test_mode = False
            self.target_sub = rospy.Subscriber('/detected_blue_cube_world', PointStamped, self.target_callback)
            rospy.loginfo("等待视觉节点发布目标位置...")
            rospy.spin()
    ...
    #     rospy.loginfo("闭合夹爪...")
    #     goal = MoveGoal(width=0.0495, speed=0.05)
    #     self.gripper_client.send_goal(goal)
    #     self.gripper_client.wait_for_result()
    #     return self.gripper_client.get_result()
    ...
    def stop_gripper(self):
        ...
        stop_client.send_goal(franka_gripper.msg.StopGoal())
    ...
    def execute_grasp(self):
        ...
        try:
            ...
            grasp_pose.position.z = self.target_position.z +0.072
            ...
            # 抓取动作
            self.grasp_object(width=0.051, force=5)
            ...
            # 释放物体
            response = input("是否释放物体? (y/n): ").strip().lower()
            if response == 'y':
                self.open_gripper()
                rospy.loginfo("物体已释放")
